...
class project
{
    ...
    function __construct( stdClass $project )
    {
        ...
        filesAndDir::build_banner_formats_data( $abs_path, $this->banner_formats, $this->project_dir, $this->global_files );
    ...
    public static function build_banner(string $dir)
    {
        ...
        $unusedCSS = [];
        $iframes = [];
        $config = json_decode(file_get_contents( $abs_path . $dir . '/banner_config.json'));
        $size = 0;
        ...
        if ($config->banner_config->template === null)
        {
            ...
            $size += helper::folder_size($abs_path . $config->banner_config->banner_dir . '/_output');
        ...
        {
            $template = json_decode(file_get_contents( $abs_path . '/banner-templates' . $config->banner_config->template));
            foreach ($template as $key => $value)
            {
                $config = json_decode(file_get_contents( $abs_path . dirname( $dir ) . '/' . $key . '/banner_config.json'));
