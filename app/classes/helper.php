...
class helper
{
    ...
    public static function get_const_dir(string $abs_path, array $array)
    {
        $return = false;
        array_walk_recursive($array, function($a) use (&$return, &$abs_path) {
            if ( file_exists( $abs_path . JS_ANIMATION_LIBRARY . constant($a)) ||
                 file_exists( $abs_path . CLIENT_BASE_SCSS . constant($a)) ||
                 file_exists( $abs_path . BANNER_TEMPLATES . constant($a)) ||
                 file_exists( $abs_path . GLOBAL_MARKUP . constant($a)) ||
                 file_exists( $abs_path . GLOBAL_FILES . constant($a)))
            {
                $return[] = constant($a);
            }
        });
    ...
    public static function get_banner_directories(string $dir)
    {
        ...
        while ($length--) {
            ...
            {
                $return[] = str_replace(substr($_SERVER['DOCUMENT_ROOT'], 0, -1), '', $dir);
    ...
    {
        $abs_path = substr($_SERVER['DOCUMENT_ROOT'], 0, -1) . $dir;
