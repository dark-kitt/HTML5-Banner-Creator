/* usage
 * colorSwap({
        id: 'redBox', set the element(s) by cl: 'class', tn: 'tag' or el: index[i] (required, string)
        delay: 2000, set the delay in milliseconds (optional/defalut 1000,number)
        fps: 30, frames per second [the loop is build with requestAnimationFrame] (optional/defalut 20, number)
        steps: 5, steps are the count value (optional/defalut 20, number)
        start: '#ec2727', start color e.g. hex, rgb, rgba, hsl or hsla (optional/defalut #000, string)
        end: '#021795', end color e.g. hex, rgb, rgba, hsl or hsla (optional/defalut #FFF, string)
        style: 'background' the style can be background-color, border-color, color, fill or stroke (required, string)
 * });
*/
var colorSwap = function(o) {
	...
	var target = gid(o.id) || gcl(o.cl) || gtn(o.tn) || o.el,
		...
		if (start.match(/hsla/g) || start.match(/rgba/g) || end.match(/hsla/g) || end.match(/rgba/g)) {
			Rstart = hsl2rgb(start)[0] || parseRGB(start)[0] || (hexToRGB(start).r === 0 ? 0 : hexToRGB(start).r);
			Gstart = hsl2rgb(start)[1] || parseRGB(start)[1] || (hexToRGB(start).g === 0 ? 0 : hexToRGB(start).g);
			Bstart = hsl2rgb(start)[2] || parseRGB(start)[2] || (hexToRGB(start).b === 0 ? 0 : hexToRGB(start).b);
			Astart = (start.match(/hsla/g)) ? hsl2rgb(start)[3] : (start.match(/rgba/g)) ? parseRGB(start)[3] : 1;

			Rend = hsl2rgb(end)[0] || parseRGB(end)[0] || (hexToRGB(end).r === 0 ? 0 : hexToRGB(end).r);
			Gend = hsl2rgb(end)[1] || parseRGB(end)[1] || (hexToRGB(end).g === 0 ? 0 : hexToRGB(end).g);
			Bend = hsl2rgb(end)[2] || parseRGB(end)[2] || (hexToRGB(end).b === 0 ? 0 : hexToRGB(end).b);
