<?php
    ...
    class filesAndDir
    {
        ...
        public static function build_banner_formats_data( array $banner = [], string $directory = '', array $global_files =[] )
        {
            ...
            if ( is_Array($banner) )
            {
                ...
                while ($length--)
                {
                    ...
                    if ( helper::array_values_int($banner[$length]) === false ) {
                        $const_dir[] = helper::get_const_dir($banner[$length]);
                        if (in_Array('NAMESPACE', $banner[$length])) {
                            $namespace = true;
                        }
                    }
                }
                $const_dir[] = helper::get_const_dir($global_files);
                ...
                foreach ( $banner as $format )
                {
                    ...
                    if ( is_Array($format) && helper::array_values_int($format) )
                    {
                        ...
                        if ( !file_exists( $abs_path . $directory . '/' . join($format, 'x') ) )
                        {
                            ...
                            $banner_config = self::build_banner_conf_file($directory . '/' . join($format, 'x'), $format, $const_dir, $namespace);

                            self::build_banner_files($banner_config);
                            self::set_base_scss($banner_config);
                            self::copy_global_files($banner_config);
                            continue;
                        }
                        else
                        {
                                 ...
                                 !file_exists( $abs_path . $directory . '/' . helper::build_duplicates($format, $count) ) )
                            {
                                ...
                                $banner_config = self::build_banner_conf_file($directory . '/' . $dub_val, $format, $const_dir, $namespace);
                    ...
                    if ($format instanceof stdClass)
                    {
                        ...
                        if ( !file_exists( $abs_path . $directory . '/' . key($format) ) )
                        {
                            ...
                            self::build_banner_formats_data( $format->{key($format)}, $directory . '/' . key($format), $global_files );
                            continue;
                        }
                        else
                        {
                                 ...
                                 !file_exists( $abs_path . $directory . '/' . helper::build_duplicates(key($format), $count) ) )
                            {
                                ...
                                mkdir( $abs_path . $directory . '/' . $dub_val );
                                self::build_banner_formats_data( $format->{key($format)}, $directory . '/' . $dub_val, $global_files );
                                continue;
                            }
        ...
        public static function build_banner_conf_file(string $path, array $format, array $files, bool $namespace)
        {
            $template = null;
            $js_animations = [];
            $base_scss = [];
            ...
            $banner_config = fopen( dirname(__DIR__) . $path . '/banner_config.json', 'w' );
        ...
        {
            $dir = dirname(__DIR__) . $config->banner_config->banner_dir;
            ...
            foreach ($config->banner_config->global_markups as $markup) {
                ...
                if ( file_exists( dirname(__DIR__) . GLOBAL_MARKUP . $markup) )
                {
                    $content = file_get_contents(dirname(__DIR__) . GLOBAL_MARKUP . $markup );
        ...
        public static function copy_global_files(stdClass $config)
        {
            $dir = dirname(__DIR__) . $config->banner_config->banner_dir;
            ...
            foreach ($global_files as $file)
            {
                ...
                if ( file_exists( dirname(__DIR__) . GLOBAL_FILES . $file) )
                {
                    $content = file_get_contents(dirname(__DIR__) . GLOBAL_FILES . $file );
                    $content = helper::match_placeholder($content, $config);
                }
            ...
            $scss_global_files = helper::custom_glob(dirname(__DIR__) . GLOBAL_FILES, '.scss');
            $js_global_files = helper::custom_glob(dirname(__DIR__) . GLOBAL_FILES, '.js');
        ...
        public static function set_base_scss(stdClass $config)
        {
            ...
            foreach ($base_scss as $file)
            {
                if ( pathinfo($file)['extension'] === 'scss' )
                {
                    if ( file_exists(dirname(__DIR__) . CLIENT_BASE_SCSS . $file) )
                    {
                        $pattern = '/\@import\b.*' . basename($file, '.scss') . '\.scss\b.*(?=\;)\;\r\n\r\n/';
                        if (!preg_match($pattern, file_get_contents( $dir . '/scss/styles.scss', 'w' )))
                        {
            ...
            $scss_files = helper::custom_glob(dirname(__DIR__) . CLIENT_BASE_SCSS, '.scss');
            foreach ( $scss_files as $file )
            {
                $pattern = '/\@import\b.*' . basename($file, '.scss') . '\.scss\b.*(?=\;)\;\r\n\r\n/';
                if ( preg_match($pattern, file_get_contents( $dir . '/scss/styles.scss', 'w' )) &&
        ...
        {
            $dir = dirname(__DIR__) . $config->banner_config->banner_dir;
            $project_dir = dirname(__DIR__) . $config->banner_config->project_dir;
        ...
        public static function build_output(stdClass $config)
        {
            ...
            {
                $js_ani .= file_get_contents( dirname( __DIR__ ) . JS_ANIMATION_LIBRARY . $js_file);
            }
            $js = '';
            ...
            $GLOBALS['scss_compiler']->setImportPaths( [ dirname( __DIR__ ) . CLIENT_BASE_SCSS, $dir . '/scss' ] );
            ...
            if ($namespace)
            {
                ...
                $output_html = $NS_content->html;
                $output_scss = $GLOBALS['scss_compiler']->compile( $NS_content->scss );
                $output_js = new Tholu\Packer\Packer($js_ani . $NS_content->js, 'Normal', true, false, true);
            }
            else
            {
                $output_html = $html;
                $output_scss = $GLOBALS['scss_compiler']->compile( $scss );
                $output_js = new Tholu\Packer\Packer($js . $js_ani, 'Normal', true, false, true);;
            }
